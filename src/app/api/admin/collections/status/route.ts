...
import { setCollectionStatus } from "@/lib/collections";
...
/** Admin: show or hide a collection from the storefront. */
export async function POST(req: NextRequest) {
  ...
  const updated = await setCollectionStatus(id, status as (typeof ALLOWED)[number]);
  if (!updated) return NextResponse.json({ error: "Collection not found" }, { status: 404 });
  await audit(me.username, "collection_status", updated.name, status);
